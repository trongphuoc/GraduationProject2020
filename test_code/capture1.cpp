...
static int uvc_streaming_1(src_v4l2_t *src, CascadeClassifier cascade, CascadeClassifier nestedCascade)
{
    long *rgb_buff;
    rgb_buff = (long *) malloc (sizeof(long) * video_out_width * video_out_heigh * 4);
	...
	printf("Getting frame from camera \n");        
       ...
       // memcpy(rgb_buff, src->buffer[src->buf.index].start, (video_out_heigh*video_out_width*4));
        ...
        Mat mSrc_BGR(szSize,CV_8UC4, rgb_buff);

        //cvtColor(mSrc, mSrc_BGR, COLOR_YUV2BGR_YUYV);
        //cvtColor(mSrc, mSrc_BGR, COLOR_YUV2BGR_YUY2);
...
int main(int argc, const char **argv)
{
    ...
    double scale;
     ...
     cv::CommandLineParser parser(argc, argv,
                                 "{help h||}"
                                 "{cascade|/opt/opencv_arm/data/haarcascades/haarcascade_frontalface_alt.xml|}"
                                 "{nested-cascade|/opt/opencv_arm/data/haarcascades/haarcascade_eye_tree_eyeglasses.xml|}"
...
//    usleep(500*1000);
  ...
  //  uvc_streaming(&uvc_src_v4l2);
    uvc_streaming_1(&uvc_src_v4l2, cascade, nestedCascade);
